...
public class FFTThread implements Runnable {
	...
	private Pair<Float, Float> m_loc;
	//private int m_fftIndex;
	
	// neural netwrok
	private boolean m_isTrained;
	private final static int NORMMAXVAL = 12000;

	public FFTThread(PApplet parent, int pt, int fftSize, int sampleRate, float[] ptX, float[] ptY, float[] ptZ, Pair<Float, Float> location) {
	...
	public void run() {
		...
		if (!m_isTrained) {
			...
			for (int i = 0; i < item.length; i++) {
				...
				colItem += String.valueOf(item[i]) + ",";
			}
			rowItem += m_loc.getX() + "\n";
			colItem += m_loc.getY() + "\n";
			
			GlobalConstants.trainingRowSet.add(rowItem);
			GlobalConstants.trainingColSet.add(colItem);

			System.out.println("insert a row in training set, output: X: " + m_loc.getX() + " Y: " + m_loc.getY());
